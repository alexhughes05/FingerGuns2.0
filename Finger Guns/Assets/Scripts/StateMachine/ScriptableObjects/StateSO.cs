	...
	public class StateSO : ScriptableObject
	{
			...
			StateMachine stateMachine, Dictionary<ScriptableObject, object> createdInstances)
		{
			...
			for (int i = 0; i < count; i++)
				actions[i] = scriptableActions[i].GetAction(stateMachine, createdInstances); //need to fix cause error is thrown if there is just an empty action. Same for conditions.
